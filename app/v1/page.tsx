...
export default function V4Home() {
  ...
  const [heroImage, setHeroImage] = useState("/og-image.png");
  ...
  useEffect(() => {
    ...
    fetch("/api/settings").then(r => r.json()).then(d => { if (d.heroImage) setHeroImage(d.heroImage); }).catch(() => {});
  }, []);
  ...
  return (
    <main style={{ backgroundColor: "#020204" }}>
      ...
      <div style={{ position: "relative", zIndex: 10, width: "100%", overflowX: "hidden" }}>
        ...
        {/* 1. Hero Section */}
        <motion.div className="v4-section" style={{ height: '100vh', opacity: heroOpacity }}>
          <h1 className="v4-title">Bal Krishna<br/>Pokharel.</h1>
          <p className="v4-subtitle">Full-Stack Developer & Designer crafting digital perfection in Kathmandu, Nepal.</p>
          ...
          <motion.div style={{ 
            ...
            <div className="v4-creative-frame">
              <div className="v4-image-inner">
                ...
                <img src={heroImage} alt="Bal Krishna Pokharel" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              </div>
            </div>
          </motion.div>
        </div>
        ...
        {/* 4. Skills Section */}
        <div className="v4-section" style={{ minHeight: '80vh' }}>
        ...
        {/* 5. Contact Section */}
        <div className="v4-section" style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', paddingBottom: '10vh' }}>
          <div style={{ width: '100%', maxWidth: '800px', margin: '0 auto', textAlign: 'center' }}>
            <h2 className="v4-heading" style={{ fontSize: 'clamp(40px, 6vw, 80px)', textShadow: '0 0 40px #06b6d4', margin: '0 0 16px' }}>Let's Build.</h2>
            <p style={{ color: "rgba(255,255,255,0.6)", fontSize: 20, marginBottom: 40 }}>Ready to create something extraordinary together?</p>
